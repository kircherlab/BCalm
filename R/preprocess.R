...
create_var_df <- function(df, map_df) {
    ...
    if (!any(df$name %in% map_df$REF) && !any(df$name %in% map_df$ALT)) {
        stop(
            "No matches found between the 'name' column in 'df' and the ",
            "'REF'/'ALT' columns in 'map_df'. Please ensure that these ",
...
.pivot_df <- function(df, id_column_name = "variant_id", allele_column_name, type) {
    if (is.null(allele_column_name)) {
        ...
        df <- df %>%
            group_by(!!sym(id_column_name), !!sym(allele_column_name)) %>%
    ...
    df_pivot <- df %>%
        pivot_wider(
            names_from = new_idx,
            values_from = matches(type, ignore.case = TRUE),
            names_prefix = "bc", id_cols = id_column_name
        ) %>%
