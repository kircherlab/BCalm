replicate_fit <- function(rep_list){
    nr_reps <- length(rep_list)
    bcs <- ncol(rep_list$rep1$DNA) # Acsess to the actual dataframe and size, assume every rep has same number of bcs?
    result <- data.frame(oligo_names = rownames(rep_list$rep1$DNA))
    for (i in 1:nr_reps){
        # create new MPRASets for each replicate
        rep_dna <- rep_list[[i]]$DNA
        rep_rna <- rep_list[[i]]$RNA
        rep_block_vec <- rep(1, each=bcs)
        ...
        top_var <- top_var[rownames(rep_dna), , drop = FALSE]
